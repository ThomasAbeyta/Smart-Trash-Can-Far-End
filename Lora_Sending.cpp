...
#line 1 "/Users/Abeyta/Documents/IoT_Projects/Lora_test/Lora_Sending/src/Lora_Sending.ino"
#include "Particle.h"
...
#line 4 "/Users/Abeyta/Documents/IoT_Projects/Lora_test/Lora_Sending/src/Lora_Sending.ino"
const unsigned long SEND_INTERVAL_MS = 6000;
const size_t READ_BUF_SIZE = 64;

const int RADIOADDRESS = 102; // 0xC1 - 0xCF
const int RADIONETWORKID = 5;    // range of 0-16
const int SENDADDRESS = 101;// 0x9B;   // address of radio to be sent to
...
void reyaxSetup(String password) {
  ...
  Serial1.printf("AT+NETWORKID=%i\r\n", RADIONETWORKID); // set the radio network
  delay(delay_MS);
  if (Serial1.available() > 0) {
  ...
  delay(delay_MS);
  if (Serial1.available() > 0) {
    Serial.printf("Awaiting Reply from TX RX MODE:\n");
    reply = Serial1.readStringUntil('\n');
    Serial.printf("Reply FROM TX RX MODE: %s\n\n\n", reply.c_str());
  }
}
